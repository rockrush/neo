...
bool add(const void* input, uint64_t length)
{
	...
	totalLength += length;
	const unsigned char* data = (const unsigned char*)input;	// byte-wise access
...
static uint64_t hash(const void* input, uint64_t length, uint64_t seed)
{
	...
	static const	uint64_t MaxBufferSize = 31 + 1;	/// temporarily store up to 31B between multiple add() calls
	uint64_t		state[4] = {seed + Prime1 + Prime2, seed + Prime2, seed, seed - Prime1};
