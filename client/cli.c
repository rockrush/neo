...
static void node_view_save_cb(GtkWidget *widget, GtkWidget *node_view)
{
	...
	dia = gtk_file_chooser_dialog_new("Export Graph", GTK_WINDOW(win), GTK_FILE_CHOOSER_ACTION_SAVE,
	...
	if (res == GTK_RESPONSE_ACCEPT) {
		...
		gboolean ret = gtk_nodes_node_view_save(GTKNODES_NODE_VIEW(node_view), fname);

		if (!ret) {
			GtkWidget *d = gtk_message_dialog_new(GTK_WINDOW(win), GTK_DIALOG_MODAL,
				GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE, "Could not open file %s", fname);
...
static void node_view_load_cb(GtkWidget *widget,  GtkWidget *node_view)
{
	...
	dia = gtk_file_chooser_dialog_new("Import Graph", GTK_WINDOW(win), GTK_FILE_CHOOSER_ACTION_OPEN,
	...
	if (res == GTK_RESPONSE_ACCEPT) {
		...
		gboolean ret = gtk_nodes_node_view_load(GTKNODES_NODE_VIEW (node_view), fname);

		if (!ret) {
			GtkWindow * win = GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(widget)));
			GtkWidget *dia = gtk_message_dialog_new(win, GTK_DIALOG_MODAL,
				GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE, "Error loading file %s", fname);
...
static void node_view_popup_menu(GtkWidget *node_view)
{
	...
	g_signal_connect(menuitem, "activate", G_CALLBACK(node_view_load_cb), node_view);
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);

	menuitem = gtk_menu_item_new_with_label("SAVE");
	g_signal_connect(menuitem, "activate", G_CALLBACK(node_view_save_cb), node_view);
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);

	menuitem = gtk_menu_item_new_with_label("Pulse");
	g_signal_connect(menuitem, "activate", G_CALLBACK(node_view_create_pulse_cb), node_view);
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);

	menuitem = gtk_menu_item_new_with_label("Step");
	g_signal_connect(menuitem, "activate", G_CALLBACK(node_view_create_step_cb), node_view);
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);

	menuitem = gtk_menu_item_new_with_label("Binary Decoder");
	g_signal_connect(menuitem, "activate", G_CALLBACK(node_view_create_node_binary_decode_cb), node_view);
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);

	menuitem = gtk_menu_item_new_with_label("Binary Encoder");
	g_signal_connect(menuitem, "activate", G_CALLBACK(node_view_create_node_binary_encode_cb), node_view);
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);

	menuitem = gtk_menu_item_new_with_label("Number Converter");
	g_signal_connect(menuitem, "activate", G_CALLBACK(node_view_create_node_convert_number_cb), node_view);
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);

	menuitem = gtk_menu_item_new_with_label("Number Display");
	g_signal_connect(menuitem, "activate", G_CALLBACK(node_view_create_node_show_number_cb), node_view);
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);

	menuitem = gtk_menu_item_new_with_label("Bitwise AND");
	g_signal_connect(menuitem, "activate", G_CALLBACK(node_view_create_node_bitwise_and_cb), node_view);
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);

	menuitem = gtk_menu_item_new_with_label("Bitwise OR");
	g_signal_connect(menuitem, "activate", G_CALLBACK(node_view_create_node_bitwise_or_cb), node_view);
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);

	menuitem = gtk_menu_item_new_with_label("Bitwise XOR");
	g_signal_connect(menuitem, "activate", G_CALLBACK(node_view_create_node_bitwise_xor_cb), node_view);
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);

	menuitem = gtk_menu_item_new_with_label("Bitwise NOT");
	g_signal_connect(menuitem, "activate", G_CALLBACK(node_view_create_node_bitwise_not_cb), node_view);
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);
...
static gboolean node_view_button_press_cb(GtkWidget *node_view, GdkEventButton *event, gpointer user_data)
{
