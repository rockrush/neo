...
static GObject *node_step_buildable_get_internal_child(GtkBuildable *buildable, GtkBuilder *builder, const gchar  *childname);

G_DEFINE_TYPE_WITH_CODE(NodeStep, node_step, GTKNODES_TYPE_NODE, G_ADD_PRIVATE (NodeStep)
						G_IMPLEMENT_INTERFACE (GTK_TYPE_BUILDABLE,	node_step_buildable_interface_init))
...
static GObject *node_step_buildable_get_internal_child(GtkBuildable *buildable, GtkBuilder *builder, const gchar *childname)
{
	NodeStep *step = NODE_STEP(buildable);
	NodeStepPrivate *priv = node_step_get_instance_private(step);
...
{
	NodeStepPrivate *priv = NODE_STEP(data)->priv;
	priv->id_trg = 0;
	gtk_nodes_node_socket_set_rgba(GTKNODES_NODE_SOCKET(priv->trg_o), &priv->rgba_trg_o);
...
static gboolean node_step_deactivate_timeout_dat_out(gpointer data)
{
	NodeStepPrivate *priv = NODE_STEP(data)->priv;
	priv->id_out = 0;
	gtk_nodes_node_socket_set_rgba(GTKNODES_NODE_SOCKET(priv->dat_o), &COL_DOUBLE);
	return G_SOURCE_REMOVE;
}
...
{
	NodeStepPrivate *priv = step->priv;

	if (priv->id_out)
		return;

	gtk_nodes_node_socket_set_rgba(GTKNODES_NODE_SOCKET(priv->dat_o), &COL_BLINK);
	priv->id_out = g_timeout_add(NODE_STEP_BLINK_TIMEOUT_MS, node_step_deactivate_timeout_dat_out, step);
}
...
static void node_step_blink_trg_out(NodeStep *step)
{
	...
	gtk_nodes_node_socket_set_rgba(GTKNODES_NODE_SOCKET(priv->trg_o), &COL_BLINK);
	priv->id_trg = g_timeout_add(NODE_STEP_BLINK_TIMEOUT_MS, node_step_deactivate_timeout_trg_out, step);
}
...
static void node_progress_bar_update(NodeStep *step)
{
	...
	buf = g_strdup_printf("%6.2f of [%g : %g]", (* priv->cur), priv->min, priv->max);
...
{
	gtk_nodes_node_socket_write(GTKNODES_NODE_SOCKET(step->priv->dat_o), step->priv->payload);
	node_step_blink_data_out(step);
}

static void node_step_trigger(GtkWidget *widget, GByteArray *payload, NodeStep *step)
{
	NodeStepPrivate *priv = step->priv;
	...
	gtk_nodes_node_socket_write(GTKNODES_NODE_SOCKET(priv->trg_o), priv->payload);
	node_step_blink_trg_out(step);
}

static void node_step_reset(GtkWidget *widget, GByteArray *payload, NodeStep *step)
{
...
static void node_step_data_connected(GtkWidget *widget, GtkWidget *source, NodeStep  *step)
{
	/* push last value */
	node_step_output(step);
}
...
{
	NodeStepPrivate *priv = NODE_STEP(step)->priv;
...
static gchar *node_step_export_properties(GtkNodesNode *node)
{
	...
	NodeStepPrivate *priv = NODE_STEP(node)->priv;
	gchar *cfg = g_strdup_printf("<child internal-child=\"sb_start\">\n"
...
static void node_step_init(NodeStep *step)
{
	...
	g_signal_connect(G_OBJECT(step), "node-func-clicked", G_CALLBACK(node_step_remove), NULL);
	gtk_nodes_node_set_label(GTKNODES_NODE (step), "Range Stepper");

	/* input sockets */
	w = gtk_label_new("Trigger");
	gtk_label_set_xalign(GTK_LABEL(w), 0.0);
	priv->trg_i = gtk_nodes_node_item_add(GTKNODES_NODE(step), w, GTKNODES_NODE_SOCKET_SINK);
	g_signal_connect(G_OBJECT(priv->trg_i), "socket-incoming", G_CALLBACK(node_step_trigger), step);

	w = gtk_label_new("Reset");
	gtk_label_set_xalign(GTK_LABEL(w), 0.0);
	priv->rst = gtk_nodes_node_item_add(GTKNODES_NODE(step), w, GTKNODES_NODE_SOCKET_SINK);
	g_signal_connect(G_OBJECT(priv->rst), "socket-incoming", G_CALLBACK(node_step_reset), step);

	/* grid containing user controls */
	grid = gtk_grid_new();
	gtk_grid_set_row_spacing(GTK_GRID(grid), 6);

	gtk_nodes_node_item_add(GTKNODES_NODE(step), grid, GTKNODES_NODE_SOCKET_DISABLE);

	/* output sockets */
	w = gtk_label_new("Output");
	gtk_label_set_xalign(GTK_LABEL(w), 1.0);
	priv->dat_o = gtk_nodes_node_item_add(GTKNODES_NODE(step), w, GTKNODES_NODE_SOCKET_SOURCE);
	gtk_box_set_child_packing(GTK_BOX(step), w, FALSE, FALSE, 0, GTK_PACK_END);
	gtk_nodes_node_socket_set_rgba(GTKNODES_NODE_SOCKET(priv->dat_o), &COL_DOUBLE);
	gtk_nodes_node_socket_set_key(GTKNODES_NODE_SOCKET(priv->dat_o), KEY_DOUBLE);
	g_signal_connect(G_OBJECT(priv->dat_o), "socket-connect", G_CALLBACK(node_step_data_connected), step);

	w = gtk_label_new("Last");
	gtk_label_set_xalign(GTK_LABEL(w), 1.0);
	priv->trg_o = gtk_nodes_node_item_add(GTKNODES_NODE(step), w, GTKNODES_NODE_SOCKET_SOURCE);
	gtk_box_set_child_packing(GTK_BOX(step), w, FALSE, FALSE, 0, GTK_PACK_END);
	/* get original colour */
	gtk_nodes_node_socket_get_rgba(GTKNODES_NODE_SOCKET(priv->trg_o), &priv->rgba_trg_o);

	/* create main controls, they need the socket reference */
	w = gtk_label_new("START");
	gtk_grid_attach(GTK_GRID(grid), w, 0, 0, 1, 1);
	w = gtk_spin_button_new_with_range(NODE_STEP_INTERVAL_MIN, NODE_STEP_INTERVAL_MAX, NODE_STEP_INTERVAL_STP);
	gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(w), TRUE);
	gtk_spin_button_set_snap_to_ticks(GTK_SPIN_BUTTON(w), TRUE);
	g_signal_connect(GTK_SPIN_BUTTON(w), "value-changed", G_CALLBACK(node_step_field_changed), &priv->min);
	...
	gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(w), TRUE);
	gtk_spin_button_set_snap_to_ticks(GTK_SPIN_BUTTON(w), TRUE);
	g_signal_connect(GTK_SPIN_BUTTON(w), "value-changed", G_CALLBACK(node_step_field_changed), &priv->max);
	...
	gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(w), TRUE);
	gtk_spin_button_set_snap_to_ticks(GTK_SPIN_BUTTON(w), TRUE);
	g_signal_connect(GTK_SPIN_BUTTON(w), "value-changed", G_CALLBACK(node_step_field_changed), &priv->stp);
	...
	g_signal_connect(G_OBJECT(w), "clicked", G_CALLBACK(node_step_clicked), step);
	gtk_grid_attach(GTK_GRID(grid), w, 0, 4, 1, 1);

	w = gtk_button_new_with_label("Reset");
	g_signal_connect(G_OBJECT(w), "clicked", G_CALLBACK(node_reset_clicked), step);
...
{
	NodeStep *step = g_object_new(TYPE_NODE_STEP, NULL);
