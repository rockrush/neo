  ...
  while (l)
	{
	  ...
	  g_object_get(child->widget, "x", &x, "y", &y,
				   "width", &width, "height", &height, "id", &id, NULL);
 ...
 */
gboolean gtk_nodes_node_view_load(GtkNodesNodeView *node_view, const gchar *filename)
{
	GtkBuilder* builder;
	GError *error = NULL;
	GSList *l;
	...
	gtk_builder_connect_signals_full(builder, gtk_nodes_node_view_connection_mapper, node_view);
